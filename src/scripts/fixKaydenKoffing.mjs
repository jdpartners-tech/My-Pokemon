// Set Kayden's Koffing (pokemonId 109) from level 40 to level 35
...
async function run() {
  ...
  const updated = party.map(p => {
    if (p.pokemonId !== 109) return p
    const newLevel = 35
    const newXp = Math.pow(newLevel, 3)
    const newMaxHp = Math.round((p.maxHp ?? 50) * (newLevel / (p.level || newLevel)))
    return { ...p, level: newLevel, xp: newXp, maxHp: newMaxHp, currentHp: newMaxHp }
  })

  await updateDoc(doc(db, 'profiles', kayden.id), { party: updated })
  console.log("Kayden's party updated:")
